...
% *** SUBFIGURE PACKAGES *** subcaption is newer and better
...
\begin{enumerate}
  ...
  \item More convolutional encoding is done, so adds parity bits on the datastream, if the 
        ...
        Burst Position Modulation-Binary Phase Shifting Keying (BPM-BPSK). The encoder is zeroed by 
        adding 2 `0' bits at the end of the PPDUs or 6 to both PHR and PSDU the latter for 
...
16-10). SYNC field scales with the number of Preamble Symbol Repetitions (PSM), that is the
...
\begin{table}[!h]
  ...
  \label{tab:PHR}
  \begin{tblr}{
    colspec={|c|c|}, % Define column specifications with vertical lines
    hline{1,Z},       % Adds horizontal lines at the first and last row
  }
...
As in figure above, the STS seed is decided by the above layer through an attribute, as the
counter, incremented each iteration of the DRBG, in order to always give a new V value.\\
...
As described in \ref{SYNC}, the STS after the 128bit random number is generated, in which each 0 
bit is a positive polarity pulse and each 1 bit is a negative polarity pulse, is spread 
by the $\delta_{L}$ function with L=8 in BPRF and L=4 in HPRF.\\
ad in Fig. \ref{fig:STS-structure}, the STS is composed of 1 to 4 active blocks, each of the
same length.
